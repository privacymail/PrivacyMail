...
def create_service_cache(service, force=False):
    ...
    for email in emails:
        cookies_per_mail.append(
            service_3p_conns.filter(mail=email, sets_cookie=True).count()
        )
    try:
...
def thesis_link_personalisation_of_services():
    ...
    print(
        "Results of comparing links between similar mails of a service (per mail pair mean)."
    )
    ...
    print(
        "{:<25}: {:<6} : {:<7}: {:<7}: {:<7}: {:<7}".format(
            "Service", "#pairs", "min", "max", "mean", "median"
        )
    )
    ratios = []
    minimums = []
    ...
    for service in services:
        ...
        (
            ...
            mean,
            median,
        ) = analyze_differences_between_similar_mails(service)
        ...
        if num_pairs == 0:
            ...
            continue
        print(
            "{:<25}: {:<6} : {:<7.2f}: {:<7.2f}: {:<7.2f}: {:<7.2f}".format(
                service_name, num_pairs, minimum, maximum, mean, median
            )
        )
    ...
    print(
        "{:<25}: {:<6} : {:<7.2f}: {:<7.2f}: {:<7.2f}: {:<7.2f}".format(
            "Total Mean", "", mean_minimum, mean_maximum, mean_mean, mean_median
        )
    )
...
def thesis_link_personalisation_of_services_only_eresources(services_to_skip=""):
    ...
    print(
        "Results of comparing links between similar mails of a service (per mail mean)."
    )
    print(
        "Ratio Total = Ratio of all embedded URLs in the HTML Body that are personalised"
    )
    print(
        "Ratio Images = Ratio of embedded image URLs in the HTML Body that are personalised"
    )
    print(
        "Ratio Links = Ratio of embedded link (anchor tags) URLs in the HTML Body that are personalised"
    )
    print(
        "Ratio Other = Ratio of other (script and link tags) embedded URLs in the HTML Body that are personalised"
    )
    print(
        "{:<25}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}: {:<12}".format(
            "Service",
            "Pers mean",
            "Mean Total",
            "Ratio Total",
    ...
    for service in services:
        ...
        for mail in service.mails():
            ...
            personalised_other_eresources = all_static_eresources.exclude(
                type="a"
            ).exclude(type="img")
            ...
            personalised_anchor_urls.append(
                personalised_anchor_urls_eresources.filter(personalised=True).count()
            )
            personalised_img_urls.append(
                personalised_img_urls_eresources.filter(personalised=True).count()
            )
            personalised_other_urls.append(
                personalised_other_eresources.filter(personalised=True).count()
            )
            personalised_links.append(
                all_static_eresources.filter(personalised=True).count()
            )
        if counter == 0:
            # print('Continue')
            continue
        ...
        if mean_images > 0:
            ratio_personalised_images = (
                statistics.mean(personalised_img_urls) / mean_images
            )
        else:
            ratio_personalised_images = 0
        mean_anchors = statistics.mean(total_anchor_urls)
        if mean_anchors > 0:
            ratio_personalised_anchors = (
                statistics.mean(personalised_anchor_urls) / mean_anchors
            )
        else:
        ...
        if mean_other_urls > 0:
            ratio_other_urls = (
                statistics.mean(personalised_other_urls) / mean_other_urls
            )
        else:
            ratio_other_urls = 0
        ...
        #     print('')
        print(
            "{:<25}: {:<12.2f}: {:<12.2f}: {:<12.2f}: {:<12.2f}: {:<12.2f}: {:<12.2f}: {:<12.2f}: {:<12.2f}:"
            " {:<12.2f}: {:<12.2f}: {:<12.2f}: {:<12.2f}".format(
                service_name,
                mean_personalised_total,
                mean_total,
                ratio_total,
                mean_personalised_img_urls,
                mean_images,
                ratio_personalised_images,
                mean_personalised_anchor_urls,
                mean_anchors,
                ratio_personalised_anchors,
                mean_personalised_other_urls,
                mean_other_urls,
                ratio_other_urls,
            )
        )
...
def chains_calculation_helper(
    eresource_set,
    show_statistics=False,
    print_long_chains=False,
    chains_lengths_to_print=5,
    analyse_syncs=False,
):
    ...
    if show_statistics:
        ...
        for service in service_dict.keys():
            ...
            num_chains_per_mail = (
                len(service_dict[service]["chain_lengths"]) / service.mails().count()
            )
            total_num_chains_per_mail += num_chains_per_mail
            service_count += 1
            print(
                "{:<25} : {:<6.2f}: {:<6.2f}: {:<6.2f}: {:<11.2f}: {:<6}".format(
                    service.name,
                    mean_length,
                    median_length,
                    max_length,
        ...
        print("\n")
        print(
            "{:<25} : {:<6.2f}: {:<6.2f}: {:<6.2f}: {:<6.2f}".format(
                "All Services Mean",
                total_means / service_count,
...
def long_chains_calculation():
    print(LONG_SEPERATOR)
    print(
        "############# The longest chains that leaks the mailaddress when viewing: #############"
    )
    # longest_chain = 0
    eresource_set = (
        Eresource.objects.filter(type="con")
        .exclude(possible_unsub_link=True)
        .exclude(is_start_of_chain=False)
    ...
    print(LONG_SEPERATOR)
    print(
        "############# The longest chains that leaks the mailaddress when clicking: #############"
    )
    # longest_chain = 0
    eresource_set = (
        Eresource.objects.filter(type="con_click")
        .exclude(possible_unsub_link=True)
        .exclude(is_start_of_chain=False)
        .exclude(is_end_of_chain=True)
        .exclude(mail_leakage__isnull=True)
    ...
    print(LONG_SEPERATOR)
    print(
        "############# The longest chains for an embedded (viewing) external resource: #############"
    )
    eresource_set = (
        Eresource.objects.filter(type="con")
        .exclude(possible_unsub_link=True)
...
def analyse_contacted_domains_from_cache():
    ...
    for service in services_queryset:
        contacted_domains_set_click = ServiceThirdPartyEmbeds.objects.filter(
            embed_type="ONCLICK"
        ).filter(service=service)
        contacted_domains_set_view = ServiceThirdPartyEmbeds.objects.filter(
            embed_type="ONVIEW"
        ).filter(service=service)
        contacted_domains_set_combined = ServiceThirdPartyEmbeds.objects.filter(
            Q(embed_type="ONCLICK") | Q(embed_type="ONVIEW")
        ).filter(service=service)
    ...
    ratio_on_view = (
        services_that_open_connections_view / num_services_receiving_mails() * 100
    )
    ratio_on_click = (
        services_that_open_connections_click / num_services_receiving_mails() * 100
    )
    ratio_on_combined = (
        services_that_open_connections_combined / num_services_receiving_mails() * 100
    )
    ...
    print(
        "{:<10} : {:<17} : {:<5.2f}".format(
            "VIEW", services_that_open_connections_view, ratio_on_view
        )
    )
    print(
        "{:<10} : {:<17} : {:<5.2f}".format(
            "CLICK", services_that_open_connections_click, ratio_on_click
        )
    )
    print(
        "{:<10} : {:<17} : {:<5.2f}".format(
            "COMBINED", services_that_open_connections_combined, ratio_on_combined
        )
    )
...
def third_party_analization_general():
        ...
        for mail in service.mails().exclude(processing_fails=3):
            ...
            for eresource in eresource_set:
                ...
                if (
                    ...
                    or tldextract.extract(settings.LOCALHOST_URL).registered_domain
                    in third_party_domain
                ):
                    continue
    ...
    for service in third_party_by_service:
        num_third_party_by_service[service] = len(third_party_by_service[service])
    s = [
        (k, num_third_party_by_service[k])
        for k in sorted(
            num_third_party_by_service, key=num_third_party_by_service.get, reverse=True
        )
    ]
    ...
    for service in third_party_by_service:
        num_third_party_by_service_clicked[service] = len(
            third_party_by_service_clicked[service]
        )
    s = [
        (k, num_third_party_by_service_clicked[k])
        for k in sorted(
            num_third_party_by_service_clicked,
            key=num_third_party_by_service_clicked.get,
            reverse=True,
        )
    ]
    for k, v in s:
    ...
    for mail in all_mails:
        ...
        if id_of_mail.count() > 0:
            ...
            for eresource in eresource_set:
                ...
                third_party_domain = resource_ext.domain + "." + resource_ext.suffix
                if (
                    tldextract.extract(settings.LOCALHOST_URL).registered_domain
                    in third_party_domain
                    or service_ext.domain in resource_ext.domain
                ):
    ...
    percent_of_mail_embed = (
        len(service_set_embedding_resources)
        / (Service.objects.all().count() - num_services_without_mails())
        * 100
    )
    print(LONG_SEPERATOR)
    print(
        "{:.2f}% of services have third party resources embedded. ({} of total {})".format(
            percent_of_mail_embed,
            len(service_set_embedding_resources),
            Service.objects.all().count() - num_services_without_mails(),
        )
    ...
    print(
        "{:.2f} third party resources on average per mail with a median of {}".format(
            statistics.mean(list_third_party_per_mail),
            statistics.median(list_third_party_per_mail),
        )
    )
    ...
    for third_party in service_by_third_party:
        num_service_by_third_party[third_party] = len(
            service_by_third_party[third_party]
        )

    s = [
        (k, num_service_by_third_party[k])
        for k in sorted(
            num_service_by_third_party, key=num_service_by_third_party.get, reverse=True
        )
    ]
    ...
    for k, v in s:
        print(
            "{:<25}: {:<5}: {}: {}".format(
                k, str(v), str(third_parties[k]), service_by_third_party[k]
            )
        )

    print(LONG_SEPERATOR)
    print(
        "{:.2f} third parties on average per mail with a median of {}".format(
            statistics.mean(third_party_count_per_mail),
            statistics.median(third_party_count_per_mail),
        )
    )
...
def address_leakage_statistics():
    ...
    leaking_resources = (
        Eresource.objects.exclude(mail_leakage=None)
        .exclude(possible_unsub_link=True)
        .filter(Q(type="con") | Q(type="con_click"))
    )
    ...
    print(
        "{:.2f}% of services have links or connections, that leak the mailaddress in plain or "
        "through a hash. ({} of total {})".format(
            percent_of_services_leaking, len(leaking_services), num_all_services
        )
    )
...
def services_leaking_to_destinations():
    ...
    for service in ser_receiving_mails:
        connections = ServiceThirdPartyEmbeds.objects.filter(service=service).filter(
            leaks_address=True
        )
        if not connections.exists():
            continue
        third_parties_tuple_list = list(
            connections.values_list("thirdparty_id").distinct()
        )
        domains = []
        ...
        print(
            "{:<20}: {:<5} : {}".format(
                service.name, len(third_parties_tuple_list), domains
            )
        )
...
def services_setting_cookies():
    ...
    for service in ser_receiving_mails:
        ...
        third_party_names_all.discard(
            tldextract.extract(settings.LOCALHOST_URL).registered_domain
        )

        third_party_names_view.discard(service.name)
        third_party_names_view.discard(
            tldextract.extract(settings.LOCALHOST_URL).registered_domain
        )

        third_party_names_click.discard(service.name)
        third_party_names_click.discard(
            tldextract.extract(settings.LOCALHOST_URL).registered_domain
        )
    ...
    print(
        "{:.2f}% of services set any cookies, {} do not.".format(
            (len(ser_receiving_mails) - len(service_without_cookies))
            / len(ser_receiving_mails)
            * 100,
            len(service_without_cookies),
        )
    )
    print(
        "{:.2f}% of services set cookies on view, {} do not.".format(
            (len(ser_receiving_mails) - len(service_without_cookies_view))
            / len(ser_receiving_mails)
            * 100,
            len(service_without_cookies_view),
        )
    )
    print(
        "{:.2f}% of services set cookies click, {} do not.".format(
            (len(ser_receiving_mails) - len(service_without_cookies_click))
            / len(ser_receiving_mails)
            * 100,
            len(service_without_cookies_click),
        )
    )
    print(
        "{:.2f}% of services set third party cookies".format(
            services_with_tp_cookies / len(ser_receiving_mails) * 100
        )
    )
...
def general_statistics():
    ...
    failed_mails_qeryset = all_mails_queryset.filter(
        processing_fails=settings.OPENWPM_RETRIES
    )
    num_failed_mails = failed_mails_qeryset.count()
    print("Number of failed mails in the Database: {}".format(num_failed_mails))

    num_failed_mails = failed_mails_qeryset.filter(
        processing_state="UNPROCESSED"
    ).count()
    print(
        "Number of failed mails in the Database while viewing: {}".format(
            num_failed_mails
        )
    )
    ...
    print(
        "Number of failed mails in the Database while clicking a link: {}".format(
            num_failed_mails
        )
    )
    ...
    no_unsubscribe_mails_qeryset = all_mails_queryset.filter(
        processing_state="NO_UNSUBSCRIBE_LINK"
    )
    num_no_unsubscribe_mails = no_unsubscribe_mails_qeryset.count()
    print(
        "Number of mails without unsub link found in the Database: {}".format(
            num_no_unsubscribe_mails
        )
    )
    ...
    print(
        "Number of services receiving Mails: {}".format(len(services_receiving_mails()))
    )
    print(
        "Number of services not receiving any mails: {}".format(
            num_services_without_mails()
        )
    )
    ...
    print(
        "Average number of identities per Service: {}".format(
            statistics.mean(number_of_identities_per_service_list)
        )
    )
    ...
    print(
        "Number of identities that do receive mails: {}".format(
            num_identities_receiving_mails
        )
    )
...
def embedded_type_statistics():
    ...
    def analyse_one_type(type):
        ...
        print(
            "{:<20}: {:<7.2f}: {:<7.2f}: {:<7}: {:<14}: {:<14}: {:<.2f}%".format(
                type_description,
                max_embeds,
                mean_embeds,
                median_embeds,
                num_with_embeds,
                num_without_embeds,
                num_with_embeds / num_all_mails * 100,
            )
        )

    print(
        "{:<20}: {:<7}: {:<7}: {:<7}: {:<14}: {:<14}: {:<14}".format(
            "Embedded Type",
            "max",
            "mean",
            "median",
...
def any_connection_overview():
    ...
    print(
        "Mails opening view connection: {}, Ratio:{:.2f}".format(
            mails_with_view_connections,
            mails_with_view_connections / mails.count() * 100,
        )
    )
    print(
        "Mails opening click connection: {}, Ratio:{:.2f}".format(
            mails_with_click_connections,
            mails_with_click_connections / mails.count() * 100,
        )
    )
    print(
        "Mails opening any connection: {}, Ratio:{:.2f}".format(
            mails_with_any_connections, mails_with_any_connections / mails.count() * 100
        )
    )
